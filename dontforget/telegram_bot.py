...
from dontforget.config import TELEGRAM_IDLE_TIMEOUT, TELEGRAM_TOKEN
...
class ChoreBot(ChatHandler):  # pylint: disable=too-many-instance-attributes
    ...
    ACTION_BUTTONS = [
        action.capitalize()
        for action in (AlarmAction.COMPLETE, AlarmAction.SNOOZE, AlarmAction.JUMP, AlarmAction.PAUSE)
    ]
    SUGGESTED_TIMES = [
        "5 min",
        "10 min",
    ...
    def __init__(self, *args, flask_app=None, **kwargs):
        ...
        for key_or_tuple, function_help in raw_mapping.items():
            ...
            if help_text:
                self.full_help.append(
                    "{commands} - {help_text}".format(commands=" or ".join(commands), help_text=help_text)
                )
    ...
    def show_active_chores(self):
        ...
        return self._show_chores(
            "Your active chores at {}:".format(local_right_now().format(DATETIME_FORMAT)), Chore.query_active()
        )

    def show_all_chores(self):
        """Show all chores."""
        # pylint: disable=no-member
        return self._show_chores("Your chores at {}:".format(local_right_now().format(DATETIME_FORMAT)), Chore.query)
